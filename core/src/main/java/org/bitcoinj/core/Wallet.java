...
public class Wallet extends BaseTaggableObject implements Serializable, BlockChainListener, PeerFilterProvider, KeyBag, TransactionBag {
    ...
    @VisibleForTesting
    final Map<Sha256Hash, Transaction> interesting;
    ...
    public Wallet(NetworkParameters params, KeyChainGroup keyChainGroup) {
        ...
        interesting = new HashMap<Sha256Hash, Transaction>();
    ...
    public boolean isConsistent() {
        ...
        try {
            ...
            int size2 = unspent.size() + spent.size() + pending.size() + dead.size() + interesting.size();
            if (size1 != size2) {
    ...
    private void processTxFromBestChain(Transaction tx, boolean forceAddToPool) throws VerificationException {
        ...
        if (hasOutputsToMe) {
            ...
            addWalletTransaction(Pool.SPENT, tx);
        } else if (tx.isInteresting(this)) {
            log.info("  tx {} ->interesting", tx.getHashAsString());
            addWalletTransaction(Pool.INTERESTING, tx);
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.NO_SUCH_TX) {
                ...
                if (result == TransactionInput.ConnectionResult.NO_SUCH_TX) {
                    ...
                    if (result == TransactionInput.ConnectionResult.NO_SUCH_TX) {
                        result = input.connect(interesting, TransactionInput.ConnectMode.ABORT_ON_CONFLICT);
                        if (result == TransactionInput.ConnectionResult.NO_SUCH_TX) {
    ...
    public Iterable<WalletTransaction> getWalletTransactions() {
        ...
        try {
            ...
            addWalletTransactionsToSet(all, Pool.INTERESTING, interesting.values());
            return all;
        } finally {
            lock.unlock();
        }
    ...
    private void addWalletTransaction(Pool pool, Transaction tx) {
        ...
        switch (pool) {
            ...
            case DEAD:
                ...
                break;
            case INTERESTING:
                checkState(interesting.put(tx.getHash(), tx) == null);
                break;
            default:
        ...
        if (!Pool.INTERESTING.equals(pool))
    ...
    EnumSet<Pool> getContainingPools(Transaction tx) {
        ...
        try {
            ...
            if(interesting.containsKey(txHash)) {
                result.add(Pool.INTERESTING);
            }
            return result;
        } finally {
            lock.unlock();
